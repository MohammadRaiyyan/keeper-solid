  ...
  return (
    <Switch>
      <Match when={local.collapsible === "none"}>
        <div
          class={cn(
            "test flex h-full w-[--sidebar-width] flex-col bg-sidebar text-foreground",
            local.class,
          )}
          {...others}
        >
      ...
      <Match when={!isMobile()}>
        ...
        >
          ...
          >
            <div
              data-sidebar="sidebar"
              class="flex size-full flex-col  group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
            >
              {local.children}
            </div>
          </div>
...
const SidebarGroup: Component<ComponentProps<"div">> = (props) => {
  ...
  return (
    <div
      data-sidebar="group"
      class={cn("relative flex w-full min-w-0 flex-col p-4", local.class)}
      {...others}
    />
  );
};
...
const SidebarGroupAction = <T extends ValidComponent = "button">(
  ...
  return (
    <Polymorphic<SidebarGroupActionProps>
      ...
      class={cn(
        "absolute right-3 top-3.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button font-medium flex w-full items-center gap-2 overflow-hidden rounded-md px-3 py-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-4 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
